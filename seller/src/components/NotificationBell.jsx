...
const NotificationBell = () => {
    ...
    // Map notification title → seller route + state
    const getNavTarget = (notification) => {
        const title = notification.title?.toLowerCase() || '';
        const orderId = notification.orderId;

        // Order-related → go to Orders page, open specific order
        if (
            title.includes('order') ||
            title.includes('new order') ||
            title.includes('packing') ||
            title.includes('packed') ||
        ...
        if (title.includes('stock') || title.includes('inventory')) {
            return { path: '/inventory', state: {} };
        }

        // Product approval / trending
        if (title.includes('product') || title.includes('approved') || title.includes('rejected') || title.includes('trending')) {
            return { path: '/products', state: {} };
        }

        // Earnings / payout
        if (title.includes('earn') || title.includes('payout') || title.includes('payment') || title.includes('revenue')) {
