...
const Earnings = ({ token }) => {
  ...
  const fetchEarnings = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${backendUrl}/api/delivery/earnings`, {
  ...
  const handleRequestPayout = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await axios.post(`${backendUrl}/api/delivery/earnings/payout`, {
        amount: payoutAmount
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">
            Track gross delivery earnings, pending delivery funds, and request bank payouts.
          </p>
        </div>
            ...
            <form onSubmit={handleRequestPayout} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={`Max ${currency}${summary.availableBalance}`}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900"
                />
                <span className="text-[11px] text-slate-400 mt-1 block">Available balance: {currency}{summary.availableBalance}</span>
              </div>
